...
# To see how annotations were generated, see figures/figure_1/panel_I/commands.sh
k27ac.dmso <- read.delim("figures/figure_1/panel_I/common_and_specific_to_CTL_annotations.txt", header=T, sep="\t")
k27ac.unc <- read.delim("figures/figure_1/panel_I/specific_to_UNC1999_annotations.txt", header=T, sep="\t")
k27me3 <- read.delim("figures/figure_1/panel_I/all_peaks_called_annotations.txt", header=T, sep="\t")

k27ac.dmso$condition <- "NT"
...
data$condition <- factor(data$condition, levels=c("H3K27me3", "UNC", "NT"))
data$myanno <- factor(data$myanno, levels=c("Distal intergenic", "Intron", "Exon/UTR", "Promoter"))

filename <- "figures/figure_1/panel_I/figure_1_I.pdf"
