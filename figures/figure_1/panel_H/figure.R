...
# See figures/figure_1/panel_H/delta_count.R to see how counts were generated
counts <- dba.load(file="H3K27me3_peaks", dir="figures/figure_1/panel_H")

# See commands.R to see how normFactors were generated
H3K27ac_factors <- read.table("figures/figure_1/panel_H/H3K27ac_normFactors.txt", header=TRUE)
H3K27me3_factors <- read.table("figures/figure_1/panel_H/H3K27me3_normFactors.txt", header=TRUE)
order <- dba.show(counts)$ID

norm <- rbind(H3K27ac_factors, H3K27me3_factors)
row.names(norm) <- norm$sample
norm.factors <- as.numeric(norm[order, ]$factor)
...
data <- data[abs(data$H3K27ac)<=300&abs(data$H3K27me3)<=1000, ]
...
filename <- "figure_1/panel_H/figure_1_H.pdf"
plot <- ggplot(data, aes(x=H3K27me3, y=H3K27ac))+
                ...
                annotate("text", x=400, y=200, label=pvalue)+
                annotate("text", x=400, y=180, label=paste0("R = ", corr))+
